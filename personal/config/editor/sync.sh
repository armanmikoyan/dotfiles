...
#      Extensions not present in an editor's marketplace are silently ignored
#      for that editor.
#
...
# Pass 2: pick up new installs from any editor.
: > "$tmp/all_installed"
for cmd in "${editors[@]}"; do
...
# Pass 3: install missing extensions; silently skip ones the marketplace doesn't have.
while IFS= read -r ext; do
  [ -z "$ext" ] && continue
  for cmd in "${editors[@]}"; do
